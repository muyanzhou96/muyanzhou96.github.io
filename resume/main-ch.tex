...
\vspace{2mm}
    ...
    \vspace{2mm}
    \item {Yilong Zou, Juan Zhai, Chunrong Fang, \textbf{Yanzhou Mu}, Jiawei Liu and Zhenyu Chen. Deep Learning Framework Testing via Heuristic Guidance Based on Multiple Model Measurements, ACM Transactions on Software Engineering and Methodology, 2026, CCF-A}
...
\resumePOR{Fast Detection, Localization, and Patch Generation for Concurrency Defects | (已完成，项目参与人)} % Position
    ...
    {} % Tenure Period
    \resumeItemListStart
    \item {开发了面向大模型训练系统的泛化测试框架，将模型分解为可复用的结构模块，在模型结构、并行策略与优化策略的联合空间中生成语义有效的模型变体。进一步研究横跨模型结构、并行、优化与后端层的配置约束，区分语义约束与实现层限制，并在保持变异意图的前提下推进深层分布式执行。基于 PTA 与 MindSpore 系框架开展跨框架差分测试。}
\resumeItemListEnd
